...
function StandingOrdersInner() {
  ...
  const load = () => {
    setLoading(true);
    Axios({ ...SummeryApi.listStandingOrders })
      .then((res) => res.data?.success && setOrders(res.data.data || []))
      .catch(() => {})
  ...
  const setStatus = async (id: string, status: string) => {
    try {
      setBusy(true);
      const res = await Axios({ ...SummeryApi.upsertStandingOrder, data: { id, status } });
      if (res.data?.success) {
  ...
  const create = async () => {
    ...
    try {
      ...
      const res = await Axios({
        ...SummeryApi.upsertStandingOrder,
        data: { intervalDays, shippingAddress, phone, items: validLines, paymentMethod: "Invoice" },
      });
      if (res.data?.success) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-4xl text-gray-900 mt-1">Standing orders</h1>
        <button
          onClick={() => setCreating((c) => !c)}
          className="px-5 py-2.5 rounded-full text-sm font-semibold bg-[#1a56db] text-white hover:bg-blue-700"
        >
          {creating ? "Cancel" : "+ New standing order"}
        </button>
      </div>
          ...
          {lines.map((l, i) => (
            <div key={i} className="flex gap-3 items-end">
              ...
              <label className="w-24 flex flex-col gap-1.5">
                <span className="text-xs font-medium text-gray-600">Cartons</span>
                <input
                  type="number"
                  min={1}
                  value={l.quantity}
      ...
      {loading ? (
        ...
        <p className="text-gray-400">No standing orders yet.</p>
      ) : (
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="border-t border-gray-50">
                  <td className="px-4 py-3 text-gray-600">Every {o.intervalDays} days</td>
                  <td className="px-4 py-3 text-gray-500">{fmt(o.nextRunAt)}</td>
                  <td className="px-4 py-3 text-gray-500">{Array.isArray(o.items) ? o.items.length : 0} line(s)</td>
                  <td className="px-4 py-3">
                    <span className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${statusColor[o.status] ?? ""}`}>
                      {o.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    {o.status === "ACTIVE" ? (
                      <button disabled={busy} onClick={() => setStatus(o.id, "PAUSED")} className="text-yellow-600 hover:underline text-xs mr-3">
                        Pause
                      </button>
                    ) : o.status === "PAUSED" ? (
                      <button disabled={busy} onClick={() => setStatus(o.id, "ACTIVE")} className="text-green-600 hover:underline text-xs mr-3">
                        Resume
                      </button>
                    ) : null}
                    {o.status !== "CANCELLED" && (
                      <button disabled={busy} onClick={() => setStatus(o.id, "CANCELLED")} className="text-red-500 hover:underline text-xs">
                        Cancel
                      </button>
                    )}
                  </td>
...
export default function TradeStandingOrdersPage() {
  return (
